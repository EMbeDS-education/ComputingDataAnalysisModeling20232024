...
tscore <- (sample.mean - 0.7 ) / (sqrt(sample.var / nrow(value)))
tscore

t1 <- qt( p = alpha/2, df = nrow(value) - 1)
accept_region <- c( - t1, t1)


```


The compyted t score is outside the critical region. So, we REJECT HO
...
pvalue <- pt( q = tscore, df = nrow(value) - 1)
pvalue


```
Pvalue is much smaller than alpha. Reject Ho


You can simply use the t.test function 
...
Test the hypothesis that the mean of choice is smaller than 0.7 (one sided test)
...
Test the hypothesis that the difference in means of choice is greater than 0.2  (one sided test)
...
Test the hypothesis that the proportion of trust is smaller than 0.5 (one sided test)
...
Test the hypothesis that the proportion of trust is smaller than 0.3 (one sided test)
...
Now we test whether it is greater than 0.1
...
Now, we test whether it is smaller
